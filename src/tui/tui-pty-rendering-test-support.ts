...
function toolFrame(rows: string[], complete: boolean) {
  const frame = text(rows);
  if (!complete) {
    return (
      frame.includes("PTY_TOOL_PARTIAL") &&
      frame.includes("Read File (running)") &&
      frame.indexOf("PTY_BEFORE_TOOL") < frame.indexOf("Read File (running)")
    );
  }
...
export async function exerciseStreamingRendering(start: StartTuiPtyFixture, timeoutMs: number) {
  await withFixture(start, {}, timeoutMs, async (fixture) => {
    ...
    await waitForSynchronizedFrameRows(
      fixture.run,
      (rows) =>
        tokens(rows).join() === TOKENS.slice(0, 64).join() && text(rows).includes("streaming"),
      timeoutMs,
    );
